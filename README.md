...
comes before and after that [[https://github.com/Nicholas-t/Cryptio/blob/master/image/block.png|alt=block]]. However, the transaction network is not a linear graph instead think of it as a 2D directed graph with every node represents
addresses *(not wallets!)*.[[https://github.com/Nicholas-t/Cryptio/blob/master/image/trans.PNG|alt=trans]]
